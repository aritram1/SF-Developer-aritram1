trigger HLPPSLX_autoPopulateParentAcc_ID on LOB_Account_Type_Priority__c(before insert,before update){
     ...
     {
         if(L.HLPP_SLX_Parent_Acc_ID__c != null)
           lid.add(L.HLPP_SLX_Parent_Acc_ID__c);
     }
     ...
     {
          if(LA[i].HLPP_SLX_Parent_Acc_ID__c != null)
             LA[i].Account__c=AccountMap.get(LA[i].HLPP_SLX_Parent_Acc_ID__c);
     }
}